...
$(document).ready(function () {
	...
	var args = {};
	args.style = {};
	args.style.styleType = 2;
	var rlt = $.pps.getCurrStyle(args);
	//alert($.toJSON(rlt));
	if(rlt.result == 0){
		currentTheme = rlt.defId;
		$.cookie('currentTheme', currentTheme, {expires: 365});
	}else{
		currentTheme = $.cookie('currentTheme') == null ? defaultTheme : $.cookie('currentTheme');
	}
	...
	var $chContainer = $('.ch-container');
	$chContainer.css("min-height",(window.screen.height - 100)+"px");
    ...
    $('ul.main-menu li a').each(function () {
		hrefStr = $($(this))[0].href;
        if ($centerFrame.length>0&&hrefStr.substring(hrefStr.indexOf('\'')+1,hrefStr.lastIndexOf('\'')) == $centerFrame.attr("src")){
    ...
    $('a.ajax-link').click(function (e) {
        ...
        if($('ul.main-menu li.active').length>0) $('ul.main-menu li.active').removeClass('active');
...
function docReady() {
    ...
    $('.noty').click(function (e) {
		...
		alert($(this).attr('data-noty-options'));
        noty(options);
    });
