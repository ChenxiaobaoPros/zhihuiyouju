...
{
    static public class ConvertUtil
    {
        /// <summary>
        /// 字节数组转换成字符串
        /// </summary>
        ...
        private static bool HasProcess(string processName)
        {
            ...
            {
                if (process.ProcessName.ToUpper() == Path.GetFileNameWithoutExtension(processName).ToUpper())
                {
                    hasProcess = true;
                    break;
                }
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetTrendMicroStatus()
        {
            string status = "9999";
            RegistryKey hkml = Registry.LocalMachine;
            RegistryKey software = hkml.OpenSubKey("SOFTWARE", true);
            RegistryKey k1 = software.OpenSubKey("TrendMicro", true);
            if (k1 != null && k1.OpenSubKey("PC-cillinNTCorp", true) != null)
            {
                //检查是否运行
                if (HasProcess("tmlisten.exe") && HasProcess("ntrtscan.exe"))
                {
                    status = "0000";
                }
                else
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetDSMClientStatus()
        {
            string status = "9999";
            RegistryKey hkml = Registry.LocalMachine;
            RegistryKey software = hkml.OpenSubKey("SOFTWARE", true);
            RegistryKey k1 = software.OpenSubKey("Huawei", true);

            if (k1 != null && k1.OpenSubKey("DSMClient", true) != null)
            {
                //检查是否运行
                if (HasProcess("DSMDaemon.exe"))
                {
                    status = "0000";
                }
                else
